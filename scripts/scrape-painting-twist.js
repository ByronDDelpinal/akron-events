...
function easternToIso(dateStr, timeStr = '19:00:00') {
...
async function ensureVenue() {
  const { data: existing } = await supabaseAdmin
    .from('venues').select('id').eq('name', 'Painting with a Twist - Fairlawn').maybeSingle()
  if (existing) return existing.id

  const { data, error } = await supabaseAdmin.from('venues').insert({
    name:          'Painting with a Twist - Fairlawn',
  ...
  if (error) { console.warn('  ⚠ Could not create Painting with a Twist venue:', error.message); return null }
  console.log('  ✚ Created venue: Painting with a Twist - Fairlawn')
  return data.id
}

async function ensureOrganizer() {
  const { data: existing } = await supabaseAdmin
    .from('organizers').select('id').eq('name', 'Painting with a Twist - Fairlawn').maybeSingle()
  if (existing) return existing.id
  ...
  if (error) { console.warn('  ⚠ Could not create PWT organizer:', error.message); return null }
  console.log('  ✚ Created Painting with a Twist - Fairlawn organizer')
  return data.id
}
