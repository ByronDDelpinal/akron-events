 ...
 * OverrideToggle
 *
 * Pill-shaped toggle next to a form field's label controlling whether the
 * field is locked against scraper overwrites. Locked → filled amber pill;
 * Unlocked → outlined muted pill. Icon + label are redundant on purpose so
 * the affordance reads when scanning fast, color-blind, or printing.
 */
export default function OverrideToggle({ field, overrides, onToggle }: OverrideToggleProps) {
...
/**
 * Inline SVG lock — the closed-shackle vs. open-shackle path swaps based on
 * `locked`. Inlining keeps the component dependency-free and crisp at any size.
 */
