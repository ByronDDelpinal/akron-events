 ...
 *   - Individual event "[Title] — [Date] in Akron, OH | Akron Pulse"
 ...
 *
 *   "Akron Art Walk — Jun 14, 2026 in Akron, OH"
 *   "Akron Art Walk — Jun 14, 2026 at Summit Artspace in Akron, OH"
 */
...
export function eventDescription(event) {
  ...
  const fallback = `${event?.title || 'Event'}${dateLabel ? ' — ' + dateLabel : ''}${venueName ? ' at ' + venueName : ''} in Akron, OH.`
...
export function hubDescription(hub) {
...
export function staticPageTitle(label) {
