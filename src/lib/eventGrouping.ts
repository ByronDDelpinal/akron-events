...
/** Minimal event shape these grouping helpers depend on. */
...
/** A rendered list item: either an event row or an injected overflow card. */
export type DayItem<E extends GroupableEvent = GroupableEvent> =
  | { type: 'event'; event: E; isRevealed: boolean }
  | { type: 'overflow'; source: string; dateKey: string; hiddenCount: number; isExpanded: boolean }
 ...
 */
export function applySourceCap<E extends GroupableEvent>(
  dayEvents: E[],
  ...
  // this source?
  const overflowEmitted: Record<string, boolean> = {}
 ...
 */
export function groupEventsByDate<E extends GroupableEvent>(
  events: E[],
): [string, E[]][] {
