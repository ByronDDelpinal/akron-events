            ...
            <tbody>
              {events.map((event) => (
                <tr key={event.id}>
                  ...
                  <td>{(event.event_categories ?? []).map((ec: Row) => ec.category).join(', ')}</td>
                  ...
                  <td>
                    {(event.event_venues ?? []).filter((ev: Row) => ev.venue).map((ev: Row, i: number) => (
                      <span key={ev.venue.id}>{i > 0 && ', '}<Link to={`/admin/venues/${ev.venue.id}/edit`} className="admin-td-link">{ev.venue.name}</Link></span>
                    ))}
                    {(event.event_venues ?? []).filter((ev: Row) => ev.venue).length === 0 && '—'}
                  </td>
