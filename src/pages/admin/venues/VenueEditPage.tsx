...
export default function VenueEditPage() {
  ...
  const [seed, setSeed] = useState<Row>(DEFAULT_VENUE)
  const [allOrgs, setAllOrgs] = useState<Row[]>([])
  const [areas, setAreas] = useState<Row[]>([])

  const fetchAreas = useCallback(async (venueId?: string) => {
    if (!venueId) return
    const { data } = await supabase.from('areas').select('*').eq('venue_id', venueId).order('name')
    setAreas((data ?? []) as Row[])
  }, [])

  useEffect(() => {
    supabase.from('organizations').select('id, name').order('name').then(({ data }) => setAllOrgs((data ?? []) as Row[]))
    if (!isNew && id) {
      ;(async () => {
        const { data } = await supabase.from('venues').select('*').eq('id', id).single()
        if (data) setSeed(data as Row)
        await fetchAreas(id)
        setReady(true)
      })()
    }
...
function VenueForm({ seed, isNew, allOrgs, venueId, areas, onAreasChange, onNavigateBack }: VenueFormProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const venueFields = {
      ...
      // Empty string from the placeholder ("Unclassified") writes null so the
      // CHECK constraint and partial index both stay happy.
      ...
      <form onSubmit={handleSubmit} className="admin-edit-form">
        ...
        <FormFieldRow>
          <FormField label="Parking Type">
            <FormSelect value={form.parking_type} onChange={(e) => setField('parking_type', e.target.value)} options={PARKING_TYPES as unknown as string[]} />
          </FormField>
...
interface VenueAreasSectionProps {
  venueId?: string
  areas: Row[]
  onAreasChange: () => void
}

function VenueAreasSection({ venueId, areas, onAreasChange }: VenueAreasSectionProps) {
...
function AddAreaForm({ venueId, onSaved, onCancel }: AddAreaFormProps) {
