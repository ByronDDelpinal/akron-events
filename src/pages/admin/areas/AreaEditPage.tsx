...
export default function AreaEditPage() {
  ...
  const [seed, setSeed] = useState<Row>(DEFAULT_AREA)
  const [allVenues, setAllVenues] = useState<Row[]>([])

  useEffect(() => {
    supabase.from('venues').select('id, name').order('name').then(({ data }) => setAllVenues((data ?? []) as Row[]))
    if (!isNew && id) {
      ;(async () => {
...
function AreaForm({ seed, isNew, areaId, allVenues, onNavigateBack }: AreaFormProps) {
