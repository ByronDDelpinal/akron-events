...
export default function FeedbackPage() {
  ...
  const [myVotes,     setMyVotes]     = useState<Set<number>>(new Set())   // post IDs I voted on
  ...
  const [formImage,   setFormImage]   = useState<File | null>(null)      // File object
  ...
  const fetchPosts = useCallback(async () => {
    ...
    try {
      ...
      if (!error && data) setPosts(data as Row[])
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (fresh) setPosts(fresh as Row[])
    } finally {
      setSubmitting(false)
    }
  }
  ...
  const renderPost = (post: Row, isResolved = false) => {
    ...
    return (
      ...
      >
        <button
          ...
          onClick={(e: MouseEvent) => { e.stopPropagation(); if (!isResolved) toggleVote(post.id) }}
        ...
        {showForm && (
          <form className="fb-form" onSubmit={handleSubmit}>
            ...
            <div
              ...
              onKeyDown={(e: KeyboardEvent) => { if (e.key === ' ' || e.key === 'Enter') { e.preventDefault(); setFormPrivate((p) => !p) } }}
            >
