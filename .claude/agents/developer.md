...
1. Git policy: NEVER push, pull, merge, reset, or force anything, and NEVER commit on or rebase the maintainer's checked-out branch. The ONLY permitted git writes are on the `agents/nightly` branch inside the dedicated worktree at `.worktrees/nightly/` (create with `git worktree add .worktrees/nightly -b agents/nightly main` if missing; plain `git worktree add .worktrees/nightly agents/nightly` if the branch exists), and they are exactly two:
   a. **First task, before any editing:** rebase `agents/nightly` onto `main` so the branch is current (`git rebase main` from inside the worktree; from the sandbox use `git --git-dir=.git/worktrees/nightly --work-tree=.worktrees/nightly rebase main`). If the rebase reports ANY conflict, run `git rebase --abort` immediately and stop — report the conflicting files instead of resolving them unattended. Verify afterward that `git merge-base main agents/nightly` equals `main`'s tip.
   b. Commits on `agents/nightly` after code-review approval.
   Commit author: `Byron Delpinal <byronddelpinal@gmail.com>`. Message format: `fix(nightly): <summary>` with body lines `Proposed-by: architect` and `Reviewed-by: code-reviewer`. All commits stay local for the maintainer to review and merge. In interactive (non-pipeline) sessions, do not commit or rebase at all; leave changes in the working tree and list suggested git commands.
